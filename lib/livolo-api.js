...
LivoloApi.prototype._createRequester = function() {
  this.apiRequester = async (path, options = {}) => {
    ...
    if (!options.skipAuth && tokenExpireTime && tokenExpireTime < new Date()) {
      await this._refreshToken();
    }
    ...
    if(!options.skipAuth && token){
      Object.assign(headers, { token, user_id, home_id })
    }
...
LivoloApi.prototype._getVerificationCode = async function() {
  ...
  const {verify_code} = await this.apiRequester(
    `/app/user/login/verify_code?user_name=${this.login}`, {skipAuth: true}
  );
  return verify_code;
};
