...
LivoloApi.prototype._reloadSwitchesData = async function() {
  ...
  /* const buttonsState = switchesButtonsState.reduce((all, button) => {
     all[button.button_id] = button.button_status;
     return all;
   }, {});
 */
...
LivoloApi.prototype._subscribeToEvents = function() {
  const { ip } = this.serverInfo;
  const { home_id } = this;
